...
MultiHMCGibbsState = namedtuple(
    "MultiHMCGibbsState", "z, hmc_states, diverging, rng_key"
)
"""
...
class MultiHMCGibbs(MCMCKernel):
    ...
    def get_diagnostics_str(self, state):
        ...
        num_steps = "/".join(["{}".format(s.num_steps) for s in state.hmc_states])
        step_size = "/".join(
    ...
    def init(self, rng_key, num_warmup, init_params, model_args, model_kwargs):
        ...
        def init_fn(init_params, key_zs):
            ...
            if self._prototype_trace is None:
                self._prototype_trace = trace(
                    substitute(
                        seed(self.model, key_zs[0]), substitute_fn=init_to_sample
                    )
        ...
        # vectorized
            ...
            key_zs = vmap(partial(random.split, num=len(self.inner_kernels) + 1))(
                rng_key
            )
